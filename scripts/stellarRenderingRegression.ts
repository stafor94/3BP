...
function testPhotosphereUsesCellularGranulationTopology() {
  ...
  assert(stellarMaterialSource.includes('const float STELLAR_CONVECTION_FREQUENCY = 3.2;'), 'large convection frequency must be centralized for Pass 3 tuning')
  assert(stellarMaterialSource.includes('const float STELLAR_GRANULE_FREQUENCY = 10.5;'), 'primary granule frequency must be centralized at a readable object-space scale')
  assert(stellarMaterialSource.includes('const float STELLAR_FINE_FREQUENCY = 26.0;'), 'fine breakup frequency must remain subordinate to primary cells')
  ...
  assert(stellarMaterialSource.includes('intergranularLane * 0.065'), 'intergranular network must contribute a bounded darker thermal lane')
  assert(stellarMaterialSource.includes('granuleCenter * 0.018'), 'granule interiors must retain a subtle hotter center lift')
  assert(stellarMaterialSource.includes('(convection - 0.5) * 0.04'), 'large convection modulation must remain lower contrast than primary cells')
  assert(stellarMaterialSource.includes('(fineBreakup - 0.5) * 0.012'), 'fine breakup must stay subordinate to cellular topology')
  ...
  assert(stellarMaterialSource.includes('float granulationContrast = clamp((granulation - 1.0) * 1.30, -0.065, 0.050);'), 'cellular topology must survive tone mapping through a bounded stellar-only modulation')
