...
import {
  ...
  stepBodies as stepStageTwoBodies,
} from './fragmentAwareEngineStageTwo'

export { MAX_NON_STELLAR_NORMALIZED_PENETRATION }
...
function getDirectionalEjectaVelocity(
  body: BodyState,
  geometry: EjectaGeometry,
  survivorVelocity: Vec3,
  rankFraction: number,
  index: number,
) {
  const relativeVelocity = subtract(body.velocity, survivorVelocity)
  ...
  // Rotate the already-budgeted solver velocity instead of adding a new kick.
  // Per-ejecta speed relative to the surviving system is unchanged; the survivor
  // correction below restores the exact represented linear momentum vector.
  return add(survivorVelocity, scale(direction, speed))
}
...
function shapeDirectionalCollisionEjecta(
  ...
  const survivorVelocity = getMassWeightedVelocity(solids, geometry.target.velocity)
  ...
  ranked.forEach((body, index) => {
    ...
    const relativeDirection = normalize(
      subtract(velocity, survivorVelocity),
      geometry.outwardNormal,
    )
