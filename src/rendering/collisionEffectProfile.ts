...
export function getCollisionEffectProfile(body: BodyState): CollisionEffectProfile {
  ...
  if (kind === 'contactFlash') {
    // Preview age encodes compression depth rather than wall-clock lifetime. It
    // must build toward contact instead of reusing the physical flash decay curve.
    const syntheticBuild = syntheticStellar ? smooth01(progress / 0.72) : 0
    const rise = syntheticStellar
      ? 0.16 + syntheticBuild * 0.84
      : physicalStellar
        ? 1
        : 0.58 + 0.42 * smooth01(progress / 0.055)
    ...
    return {
        ...
        : physicalStellar
          ? stellarOutcome === 'hitAndRun'
            ...
            : stellarOutcome === 'partialDisruption'
              ...
              : 1
          : 0.94,
      innerGlow: syntheticStellar ? 0.72 : 1,
      outerGlow: syntheticStellar ? 0.34 : physicalStellar ? 0.48 : 0.3,
      visualRadius: syntheticStellar
        ? THREE.MathUtils.clamp(body.radius * (0.82 + progress * 0.2), 0.052, 0.15)
        : physicalStellar
      ...
      brightness: syntheticStellar
        ? (visual?.brightness ?? 1.45) * (0.74 + syntheticBuild * 0.26)
        : (visual?.brightness ?? (physicalStellar ? 2.35 : 1.55)) * outcomeBrightnessBoost,
      turbulence: visual?.turbulence ?? (physicalStellar ? 0.68 : 0.18),
      cooling: syntheticStellar ? progress * 0.12 : smooth01(progress),
    }
  }
  ...
  if (kind === 'compressionShear') {
    ...
    const rise = syntheticStellar
      ? 0.1 + syntheticBuild * 0.9
      : smooth01(progress / (physicalStellar ? 0.045 : stellar ? 0.08 : 0.12))
    const decay = syntheticStellar
      ? 1
      : Math.pow(1 - progress, physicalStellar ? 1.18 : stellar ? 1.5 : 1.7)
    ...
    return {
          ...
          : stellar
            ? stellarOutcome === 'merge'
              ...
              : stellarOutcome === 'partialDisruption'
                ...
                : 0.66
            : 0.7,
      innerGlow: syntheticStellar ? 0.64 : physicalStellar ? 0.9 : stellar ? 0.82 : 0.72,
      outerGlow: syntheticStellar ? 0.24 : physicalStellar ? 0.3 : stellar ? 0.24 : 0.18,
      ...
      brightness: syntheticStellar
        ...
        : (visual?.brightness ?? (physicalStellar ? 1.32 : 1.18)) * outcomeBoost,
  ...
  if (kind === 'stellarPlasma') {
    ...
    return {
      ...
      progress,
      fadeAlpha: linger,
      baseOpacity: syntheticStellar ? 0.72 : physicalStellar ? 0.86 : 0.78,
      innerGlow: syntheticStellar ? 0.68 : physicalStellar ? 0.9 : 0.82,
      outerGlow: syntheticStellar ? 0.22 : physicalStellar ? 0.26 : 0.21,
      visualRadius: physicalStellar
      ...
      brightness: syntheticStellar
        ? (visual?.brightness ?? 1.28) * (0.72 + syntheticBuild * 0.28)
        : (visual?.brightness ?? (physicalStellar ? 1.4 : stellar ? 1.28 : 1.12)) *
          (1 - progress * (physicalStellar ? 0.12 : 0.18)),
      turbulence: visual?.turbulence ?? (physicalStellar ? 0.7 : 0.62),
      cooling: syntheticStellar ? progress * 0.1 : Math.pow(progress, 1.18),
    }
  }
