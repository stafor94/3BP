...
- collision camera가 merge 이후 이미 해제된 tracking 상태를 잠시 가리고 있다가 종료 순간 천체가 화면 밖으로 이탈하던 문제를 수정했습니다.
...
- 흡수 충돌의 작은 천체는 전체 구면 dissolve 대신 접촉면 쪽으로 압축·소멸되는 짧은 전환을 사용하도록 분리했습니다.
