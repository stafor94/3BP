...
### Fixed
- 항성 표면의 온도색을 유지하면서 저주파 질감과 중심-가장자리 명암을 복원했습니다.
- 단순 근접 시 충돌 외피가 켜지던 경로와 회색 외곽 셸을 제거하고, 변형된 충돌 외피에는 실제 깊이 윤곽으로 코로나를 가리도록 수정했습니다. 숨겨진 구형 본체의 원형 마스크가 남겨 검은 초승달이 보이던 합성 불일치를 바로잡았습니다.
- 항성 접촉의 가는 밝은 ridge를 부드러운 유색 패치로 바꾸고 중첩 발광의 백색 포화를 줄였습니다. 잔열은 속이 빈 고리 대신 깊이 검사를 따르는 확산 구름으로 표시합니다.
- 충돌 외피의 광채를 형상에 맞추고 정상 코로나로 부드럽게 전환하며, 방출 가스 리본을 경계가 부드러운 plume으로 정리했습니다.
- 접촉 효과의 천체 ID/계보 기반 부착, 방출 가스 이력의 불연속 분절, 종류별 시각 크기·수명 적용을 정리했습니다. 물리 상태·충돌 결과·질량·운동량은 변경하지 않습니다.

### Verification
- 기존 correctness 및 production 시각 검증을 사용하고, 평평한 광구를 거부하는 중심-중간 반경 밝기 차이 검사와 코로나 상태 격리·복귀 회귀를 보강했습니다. 최종 실행 결과와 남은 제한은 PR #160 및 인계 문서에 기록합니다.
...
### Changed
- 항성 충돌 표현을 시뮬레이션 시간에 연결된 3D 외피로 전환했습니다. 합체에서는 두 표면의 연결부와 부피 이동을 거쳐 비대칭 잔해로 수렴하며, 부분 파괴·스침 충돌에서는 두 생존체를 유지합니다.
- 충돌 효과 생성·갱신·폐기를 production 프레임 루프의 단일 소유 경로로 통합하고, 항성 가림막과 양방향 shock sprite를 해당 경로에서 제거했습니다.
- 항성 가스를 색이 남는 넓은 밀도 분포로 바꾸고 물리적 age에 따라 방출·확산·냉각합니다. 항성 전용 분출 방향을 넓히며 기존 질량·선운동량 보정은 유지합니다.

### Verification
- Production 물리 경로의 외피 상태 전이·생존체·일시정지·법선·질량 회귀 검사를 추가했습니다. 밝은 가림막을 요구하던 시각 검사를 production 충돌 캡처와 실루엣 경계 검사로 교체했습니다.
- 실제 영상 A/B 및 모바일 성능 검증 결과는 작업 검증 문서를 참조합니다. 자동 테스트만으로 시각 품질을 확정하지 않습니다.

## [0.26.0] - 2026-09-08

### Added
- 천체 2·3·4개 초기 조건에 서로 다른 질량과 온도색의 항성으로 구성한 쌍성, 계층형 삼중성, 이중 쌍성 공전 프리셋을 추가했습니다. 기존 온도색 계산을 그대로 사용하며 모든 항성이 초기 속도를 가지고 움직입니다.
...
### Fixed
- 상단 선택 천체 정보를 한 줄로 유지하고 좁은 모바일 폭에서는 보조 라벨만 숨겨 진화 단계/표면 프리셋 값이 유지되도록 수정했습니다.

## [0.25.0] - 2026-09-08

### Changed
- 천체 추적 상단 정보에 항성 진화 단계 또는 행성·위성 표면 프리셋을 표시한다.
- 왼쪽 천체 선택 시 하단에서 해당 천체 정보만 펼치며 패널 자체의 접힘 상태는 유지한다.
- 모바일에서 선택 천체 정보를 별도 줄에 배치해 프리셋 이름을 읽을 수 있게 한다.
